...
public class Main {
    ...
    Parameters for compact constructor are defined implicitly and assignment to corresponding fields are also implicit at the end of compact constructor.
    Example
...
Rules:
 ...
 Record class is implicitly final and can't be abstract. State => API. We don't want any subclass to change our api because then State => API wouldn't work.
 Record headers define record fields and they are implicitly final. Because record is immutable data carrier.
 Record can't have other instance fields except those that are defined in record headers. And cannot contain instance initializers.
 ...
 record can declare nested types, including nested record classes. Record if nested is implicitly static. This was done to not add any state behind the scenes for nested record
 ...
 local classes are always not static. You can't define them as static. So local class that is defined in the method can always access enclosing method variables and instance fields of enclosing class.
...
inner classes now allows static nested classes.
    ...
    class A {
        ...
        static record B() {} // now inner classes allows static nested classes including record enum and interfaces.
        static class C {}
        static enum MyEnum {ONE, TWO} // implicitly static
        static interface I {} // implicitly static
    }
}

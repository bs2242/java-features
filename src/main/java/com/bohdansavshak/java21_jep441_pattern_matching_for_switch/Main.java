...
public class Main {
	...
	public static void main(String[] args) {
		/*
		 * before java 21 switch works only on char, byte, short and int(autoboxed
		 * versions too) + enum and string work on constants only.
		 *
		 */
	}
	...
	/*
	 * Switches and null before java 21 if you pass null to switch you will get
	 * NullPointerException. But know you can create another case label specifically for
	 * null check.
	 *
	 * if there would be no case null in switch then you would get NullPointerException if
	 * case null is there then right block to it will be executed. default doesn't mean it
	 * matches null just to maintain backward compatibility.
	 */
	 ...
	 *
	 * before java 21 enum must be a variable that is passed to switch and switch case
	 * must contain only enum names.
	 */
	...
	static void exhaustivnessWithEnum() {
		...
		/*
		 * If we omit the default clause then we will discover at compile time if we have
		 * forgotten a case label, rather than finding out at run time — and maybe not
		 * even then. In conclusion: An exhaustive switch without a match-all clause is
		 * better than an exhaustive switch with one, when possible.
		 */
	 ...
	 *
	 * SwitchLabel: case CaseConstant { , CaseConstant } case null [, default] case
	 * Pattern [ Guard ] default
	 */
	...
	static void testScope2(Object obj) {
		switch (obj) {
			...
			case Integer i -> throw new IllegalStateException("Invalid Integer argument: " + i.intValue());
			default -> {
				break;
			}
		}
	...
	/*
	 * A guarded pattern case label dominates another pattern case label (guarded or
	 * unguarded) only when both the former's pattern dominates the latter's pattern and
	 * when its guard is a constant expression of value true. For example, the guarded
	 * pattern case label case String s when true dominates the pattern case label case
	 * String s. We do not analyze the guarding expression any further in order to
	 * determine more precisely which values match the pattern label — a problem which is
	 ...
	 *
	 * The requirement of exhaustiveness applies to both pattern switch expressions and
	 * pattern switch statements. To ensure backward compatibility, all existing switch
	 * statements will compile unchanged. But if a switch statement uses any of the switch
	 * enhancements described in this JEP then the compiler will check that it is
	 * exhaustive.
	 *
	 * More precisely, exhaustiveness is required of any switch statement that uses
	 * pattern or null labels or whose selector expression is not one of the legacy types

...
public class Introduction {
	...
	public static void main(String[] args) {
		...
		// Let's say you have Collections class which exists already in java 4 but since 5
		// there are
		// generics added.
		...
		// Collections.getList().
		// Imaginary method that was existing in java 4 bug since 5 it now returns generic
		// List.
		// So if you have used this method it means that you expect just
		// List but since java 5 you would get parameterized type List<Something> which
		// will break
		// you
		// code
		// if this feature of assigning of parameterized type to raw type wouldn't be
		// supported.
	...
	/*
	 * type parameter type argument generic type generic type invocation parameterized
	 * type Raw type ✔ generic methods ✔ Bounded type parameters(Multiple Bounds) ✔
	 * Generic Methods and Bounded type parameters ✔ Generics, Inheritance and Subtypes ✔
	 *
	 * type inference Wildcards Type Erasure Restrictions on Generics
	 *
	 *
	 * Allows you to use type as parameter on class and interface definition and method
	 * definition. So you can reuse the same code multiple time but for different types.
	 * Generics allow types to be parameters when defining classes, interfaces and
	 * methods.
	 *
	 * Benefits of generics: - compile time checks - don't need to do any casting java
	 * does it for you - you can write generic code that can be reused multiple times
	 *
	 * naming conventions: E - for element K - for key T - for type V - for value N - for
	 * number S, U, V -- 2nd, 3rd, 4th types Only letter because otherwise it would be
	 * difficult to tell difference between type variable and class, interface name.
	 *
	 *
	 * Raw types Raw type is a generic class/interface that you define without any type
	 * arguments. Non-generic class or interface type is not a raw type.
	 *
	 * Generic methods - introduce their own type parameters. static and non-static is
	 * allowed as well as generic class constructors. Scope of type parameters in this
	 * case would be the scope of method. You should define list of type parameters in <>
	 * before return type of the method.
	 *
	 * Bounded type parameters If you want to restrict the types that can be used as type
	 * arguments then you use Bounded type parameters. Not only you limit which type can
	 * be used, but also you can call methods on bounded type in the scope of type param.
	 * Multiple Bounds <T extends B1 & B2 & B3> T is subtype of all three above. If one of
	 * the bounds a class it must be specified as first. In this case B1 must be a class
	 * of T extends from a class.
	 *
	 *
	...
	public static class Util {
		...
		public static void main(String[] args) {
			...
			boolean isSame = Util.compare(p1, p2); // Explicit was of telling java which
													// type arguments
			// use for static generic method.
			boolean isSame2 = Util.compare(p1, p2); // type inference java implicitly
													// define types
		}
	...
	public static class BoxAsGenericType<T> {

		// <T> in this case is called type parameter
		// or type variable.
	...
	public static class OrderedPair<K, V> implements Pair<K, V> {
		...
		public static void main(String[] args) {
			OrderedPair<String, BoxAsGenericType<Integer>> p = new OrderedPair<>("hello",
					new BoxAsGenericType<Integer>()); // parameterized type
		}
	...
	public static class GenericHierarchy {
		...
		public static void main(String[] args) {
			...
			// Integer extends Number. So in every place where Number is expected you can
			// pass
			// Integer.

			// But when BoxAsGenericType<Number> is expected you can't pass
			// BoxAsGenericType<Integer>
			// because both have superclass Object and there is no direct link between
			// them.
			// also because List<E> extends from Collection<E> it means when Collection<E>
			// is
			// expected you
			// can pass in List<E>
		...
		// so technically by doing generic type invocation like ListWithPayLoad<String,
		// Integer> it
